#!/usr/bin/env bash
# v4 pipeline runner — phase 1 → top-3 → 3×(phase 2 + GSM8K) → pick winner.
#
# Identical flow to run_v3.sh; only the names differ (v4 configs, v4
# checkpoint dirs, v4 eval JSONs) so v3 artifacts are preserved
# side-by-side. v4 trains on the scheduled-sampling cache
# (cache_v4_20k/llada).
#
#   SKIP_PHASE1=1 ./run_v4.sh      # phase 1 already done
#   ONLY_PHASE1=1 ./run_v4.sh      # stop after phase 1
#
...
LOG_DIR="${LOG_DIR:-${REPO_ROOT}/logs/v4}"

PHASE1_CFG="delta_model/configs/v4_phase1_delta_llada_variant_c.yaml"
PHASE2_CFG="delta_model/configs/v4_phase2_conf_llada_variant_c.yaml"
PHASE1_CKPT_DIR="${REPO_ROOT}/ckpts/v4_phase1_delta_llada_variant_c"

cd "$REPO_ROOT"
mkdir -p "$LOG_DIR" eval_results

echo "[v4] repo=$REPO_ROOT  fast_dllm=$FAST_DLLM_PATH"

echo "[v4] py_compile preflight..."
...
echo "[v4] py_compile OK"

# --- Stage 1: phase-1 delta training ---------------------------------------

if [[ "${SKIP_PHASE1:-0}" != "1" ]]; then
  echo "[v4] === Stage 1: phase-1 delta training ==="
  ...
  echo "[v4] SKIP_PHASE1=1 — using existing $PHASE1_CKPT_DIR"
fi

mapfile -t PHASE1_CKPTS < <(ls -1 "$PHASE1_CKPT_DIR"/best_delta_gsm8k_step*.pt 2>/dev/null || true)
if [[ ${#PHASE1_CKPTS[@]} -eq 0 ]]; then
  echo "[v4] FAIL: no best_delta_gsm8k_step*.pt in $PHASE1_CKPT_DIR" >&2
  exit 1
fi
echo "[v4] phase-1 top-${#PHASE1_CKPTS[@]} checkpoints:"
printf '       %s\n' "${PHASE1_CKPTS[@]}"

if [[ "${ONLY_PHASE1:-0}" == "1" ]]; then
  echo "[v4] ONLY_PHASE1=1 — stopping after phase 1."
  exit 0
fi
...
for p1 in "${PHASE1_CKPTS[@]}"; do
  i=$((i + 1))
  tag="cand${i}"
  p2_dir="${REPO_ROOT}/ckpts/v4_phase2_conf_${tag}"
  echo "[v4] === Stage 2 (${tag}): phase-2 conf training from $(basename "$p1") ==="
  ...
  if [[ ${#conf_ckpts[@]} -eq 0 ]]; then
    echo "[v4] WARN: no checkpoint produced for ${tag}; skipping its sweep" >&2
    continue
  fi
  ...
  for cc in "${conf_ckpts[@]}"; do
    ci=$((ci + 1))
    out_json="eval_results/v4_${tag}_c${ci}_sweep.json"
    echo "[v4] === Stage 3 (${tag} c${ci}): GSM8K sweep on $(basename "$cc") ==="
...
echo "[v4] === done. per-candidate best hybrid accuracy ==="
...
if [[ ${#sweep_jsons[@]} -ge 1 ]]; then
  echo "[v4] overlay plot → plots/v4_candidates_sweep.png"
  python -m delta_model.eval.plot_sweep "${sweep_jsons[@]}" \
      --out plots/v4_candidates_sweep.png || true
fi
